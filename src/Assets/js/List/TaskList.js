...
export class TaskList {
    ...
    async getItems() {
        ...
        await gapi.client.tasks.tasks.list({
            ...
            for (let task of response.result.items) {
                tasks.push(new TaskItem(task.id, task.title, task.notes, Date(Date.parse(task.due))));
            }
        });
    ...
    async getItem(id) {
        await gapi.client.tasks.tasks.get({
            ...
            return new TaskItem(response.result.id, response.result.title, response.result.notes, Date(Date.parse(response.result.due)));
        });
    }
